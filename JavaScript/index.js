...
document.getElementById('subscribe-form').addEventListener('submit', function(e) {
  ...
  if (input.value.includes('@')) {
    ...
    input.value = '';
    setTimeout(() => document.getElementById('sub-success').classList.add('hidden'), 3000);
  }
});
...
const defaultConfig = {
  brand_name: 'Zafran',
  tagline: 'Pure Afghan Saffron â€” harvested with care, delivered with trust. The essence of authenticity in every thread.',
...
function applyConfig(config) {
  ...
  document.getElementById('email-text').textContent = config.email || defaultConfig.email;
  document.getElementById('phone-text').textContent = config.phone || defaultConfig.phone;
  document.getElementById('location-text').textContent = config.location || defaultConfig.location;
  ...
  document.getElementById('footer-root').style.background = `linear-gradient(160deg, ${bg} 0%, ${lighten(bg, 10)} 40%, ${bg} 100%)`;
  ...
  document.querySelectorAll('.nav-link').forEach(el => el.style.color = txt);
...
(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'9e806a16b61a292c',t:'MTc3NTQ3NDU0My4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();
...
function setLang(lang) {
  ...
  document.getElementById('brand-tagline').textContent = document.getElementById('brand-tagline').getAttribute(attr);

  ['lang-en', 'lang-dr', 'mob-lang-en', 'mob-lang-dr'].forEach(id => {
    const btn = document.getElementById(id);
    if (btn) {
      btn.classList.toggle('active', (lang === 'en' && id.includes('en')) || (lang === 'dr' && id.includes('dr')));
    }
  });
